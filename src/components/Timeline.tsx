    ...
    <ol className="relative space-y-10 border-l border-line pl-8">
      {steps.map((step) => (
        <li key={step.title} className="relative">
          <span className="absolute -left-[35px] top-1 h-3 w-3 rounded-full border-2 border-accent-ink bg-accent" />
          <p className="font-display text-xl font-bold text-accent-ink">{step.year}</p>
